...
/**
 * Agent Swarm Workflow Progress Component
 *
 * Custom workflow progress component for the 9-step Agent Swarm workflow.
 * Displays progress through: Upload PRD, Generate PRD Analysis, Create Data Model,
 * Generate Backlog, Create Sprint Plan, Assign Agents, Agent Collaboration,
...
interface AgentSwarmWorkflowProgressProps {
  currentStep: number;
...
export default function AgentSwarmWorkflowProgress({
  currentStep,
  completedSteps,
  onStepClick,
  className,
}: AgentSwarmWorkflowProgressProps) {
  // Calculate step statuses based on current step and completed steps
  const steps: WorkflowStep[] = workflowSteps.map((step) => {
  ...
  const handleStepClick = (step: WorkflowStep) => {
    // Only allow clicking completed steps or steps before current
