...
export default function PlanManagementClient() {
  ...
  const [availablePlans, setAvailablePlans] = useState<Array<{id: string, name: string, price: number, features: string[]}>>([]);
  ...
  useEffect(() => {
    const fetchPlanData = async () => {
      try {
        ...
        // Fetch current plan and available plans in parallel
        const [plan, plansResponse] = await Promise.all([
        ...
        setCurrentPlan({
          name: plan.name,
          price: `${plan.currency || 'USD'} ${plan.price.toFixed(2)}`,
          status: plan.status,
          current_period_end: plan.current_period_end,
          features: plan.features || []
        });

        // Map available plans for display
        setAvailablePlans(
          plansResponse.map(p => ({
            id: p.id,
            name: p.name,
  ...
  const isCancelled = currentPlan?.status === 'cancelled' || currentPlan?.status === 'past_due';
                          ...
                          <div className="flex justify-between items-start mb-3">
                            <div>
                              ...
                              <p className="text-sm text-muted-foreground">
                                ...
                                <span className="text-xs">{plan.name.includes('Yearly') ? 'year' : 'month'}</span>
                              </p>
                            </div>
                            {isCurrent ? (
