...
class AuthService {
   ...
   * Uses form-urlencoded format as required by the OAuth2 login API
   */
  async login(email: string, password: string): Promise<LoginResponse> {
    try {
      ...
      const fullURL = `${this.baseURL}/api/v1/public/auth/login`;
      ...
      if (data.access_token) {
        ...
        try {
          const userInfoResponse = await fetch(`${this.baseURL}/api/v1/public/auth/me`, {
            headers: {
  ...
  async handleOAuthCallback(code: string, state?: string | null): Promise<LoginResponse> {
    try {
      ...
      const response = await fetch(`${this.baseURL}/api/v1/public/auth/github/callback`, {
        method: 'POST',
        headers: {
          'Content-Type': 'application/json',
        },
  ...
  async getCurrentUser(): Promise<UserProfile | null> {
    try {
      ...
      const response = await fetch(`${this.baseURL}/api/v1/public/auth/me`, {
        headers: {
  ...
  async logout(): Promise<void> {
    try {
      const token = this.getAccessToken();
      if (token) {
        await fetch(`${this.baseURL}/api/v1/public/auth/logout`, {
          method: 'POST',
          headers: {
            'Authorization': `Bearer ${token}`,
          },
  ...
  async refreshAccessToken(): Promise<string | null> {
    ...
    try {
      const response = await fetch(`${this.baseURL}/api/v1/public/auth/refresh`, {
        method: 'POST',
        headers: {
          'Content-Type': 'application/json',
        },
  ...
  async verifyEmail(token: string): Promise<{ message: string }> {
    try {
      const response = await fetch(`${this.baseURL}/api/v1/public/auth/verify-email?token=${encodeURIComponent(token)}`, {
        method: 'GET',
        headers: {
          'Content-Type': 'application/json',
        },
  ...
  async resetPassword(token: string, newPassword: string): Promise<{ message: string }> {
    try {
      const response = await fetch(`${this.baseURL}/api/v1/public/auth/reset-password`, {
        method: 'POST',
        headers: {
          'Content-Type': 'application/json',
        },
  ...
  async requestPasswordReset(email: string): Promise<{ message: string }> {
    try {
      const response = await fetch(`${this.baseURL}/api/v1/public/auth/forgot-password`, {
        method: 'POST',
        headers: {
          'Content-Type': 'application/json',
        },
