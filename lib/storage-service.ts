...
class StorageService {
   ...
   * Buckets map to project-scoped file listings via /api/v1/projects/{project_id}/files
   * Refs #687
   */
  ...
  async listFiles(params: ListFilesParams = {}): Promise<FilesListResponse> {
    ...
    const endpoint = `/api/v1/projects/${projectId}/files${
  ...
  async uploadFile(file: File, metadata?: Record<string, unknown>): Promise<FileUploadResponse> {
    ...
    const response = await apiClient.post<FileUploadResponse>(
      `/api/v1/projects/${this.defaultProjectId}/files/upload`,
      formData,
      {
        headers: {
  ...
  async getFileStats(): Promise<FileStatsResponse> {
    ...
    try {
      const response = await apiClient.get<FileStatsResponse>(
        `/api/v1/projects/${projectId}/files/stats/summary`
      );
      return response.data;
    } catch (error) {
  ...
  async updateFileMetadata(
    ...
    const response = await apiClient.post<FileMetadataResponse>(
      `/api/v1/projects/${this.defaultProjectId}/files/metadata`,
      {
        file_id: fileId,
        metadata,
      }
  ...
  async getFile(fileId: string): Promise<StorageFile> {
    const response = await apiClient.get<StorageFile>(
      `/api/v1/projects/${this.defaultProjectId}/files/${fileId}`
    );
    return response.data;
  }
  ...
  async getFileContent(fileId: string): Promise<string> {
    const response = await apiClient.get<Blob>(
      `/api/v1/projects/${this.defaultProjectId}/files/${fileId}/content`,
      {
        headers: {
          Accept: '*/*',
        },
  ...
  async downloadFile(fileId: string, filename?: string): Promise<void> {
    const response = await apiClient.get<Blob>(
      `/api/v1/projects/${this.defaultProjectId}/files/${fileId}/download`,
      {
        headers: {
          Accept: '*/*',
        },
  ...
  async listDatabaseFiles(params: ListFilesParams = {}): Promise<FilesListResponse> {
    ...
    const endpoint = `/api/v1/zerodb/${this.defaultProjectId}/database/files${
      queryParams.toString() ? '?' + queryParams.toString() : ''
    }`;
    const response = await apiClient.get<FilesListResponse>(endpoint);
    return response.data;
  ...
  async createDatabaseFile(file: File): Promise<FileUploadResponse> {
    ...
    const response = await apiClient.post<FileUploadResponse>(
      `/api/v1/zerodb/${this.defaultProjectId}/database/files`,
      formData,
      {
        headers: {
          'Content-Type': 'multipart/form-data',
  ...
  async getDatabaseFileStats(): Promise<FileStatsResponse> {
    const response = await apiClient.get<FileStatsResponse>(
      `/api/v1/zerodb/${this.defaultProjectId}/database/files/stats`
    );
    return response.data;
  }
  ...
  async getDatabaseFile(fileId: string): Promise<StorageFile> {
    const response = await apiClient.get<StorageFile>(
      `/api/v1/zerodb/${this.defaultProjectId}/database/files/${fileId}`
    );
    return response.data;
  }
  ...
  async downloadDatabaseFile(fileId: string, filename?: string): Promise<void> {
    const response = await apiClient.get<Blob>(
      `/api/v1/zerodb/${this.defaultProjectId}/database/files/${fileId}/download`,
      {
        headers: {
          Accept: '*/*',
        },
  ...
  async getPresignedUrl(fileId: string, expiresIn?: number): Promise<PresignedUrlResponse> {
    const response = await apiClient.post<PresignedUrlResponse>(
      `/api/v1/zerodb/${this.defaultProjectId}/database/files/${fileId}/presigned-url`,
      {
  ...
  async uploadDatabaseFile(file: File): Promise<FileUploadResponse> {
    ...
    const response = await apiClient.post<FileUploadResponse>(
      `/api/v1/zerodb/${this.defaultProjectId}/database/storage/upload`,
      formData,
      {
        headers: {
          'Content-Type': 'multipart/form-data',
