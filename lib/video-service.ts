...
const videoService = {
  ...
  async uploadVideo(request: UploadVideoRequest): Promise<Video> {
    ...
    const response = await apiClient.post<Video>('/api/v1/video/upload', formData, {
      headers: {
  ...
  async getVideoLibrary(params?: VideoLibraryParams): Promise<VideoLibraryResponse> {
    ...
    const endpoint = `/api/v1/video/library${queryParams.toString() ? `?${queryParams.toString()}` : ''}`;
  ...
  async getVideoAnalytics(videoId: string): Promise<{
    ...
    const response = await apiClient.get<{
      ...
      completionRate: number;
      engagementScore: number;
    }>(`/api/v1/video/${videoId}/analytics`);
    return response.data;
  },
};
