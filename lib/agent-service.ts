...
const agentService = {
  ...
  async getAgents(): Promise<Agent[]> {
    const response = await apiClient.get<{ agents: Agent[] }>('/api/v1/agents');
  ...
  async createAgent(request: CreateAgentRequest): Promise<Agent> {
    const response = await apiClient.post<Agent>('/api/v1/agents', request);
  ...
  async getAgentLogs(agentId: string, runId?: string): Promise<AgentLog[]> {
    const endpoint = runId
      ? `/api/v1/agents/${agentId}/logs?runId=${runId}`
  ...
  async getTemplates(): Promise<AgentTemplate[]> {
    const response = await apiClient.get<{ templates: AgentTemplate[] }>('/api/v1/agents/templates');
