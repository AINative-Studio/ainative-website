...
class AdminService {
  ...
  async getSystemHealth(): Promise<SystemHealth> {
    const response = await apiClient.get<MetricsHealthResponse>('/api/v1/metrics/health');
  ...
  async getSystemMetrics(): Promise<SystemMetrics> {
    const response = await apiClient.get<MetricsSummaryResponse>('/api/v1/metrics/summary');
    // Transform backend response to expected format
    return {
      cpu: {
