...
export function useModelEvaluation(modelId: string, enabled: boolean = true) {
  return useQuery({
    ...
    queryFn: async (): Promise<ModelEvaluation> => {
      return qnnApiClient.getEvaluation(modelId);
    },
...
export function useRunEvaluation() {
  ...
  return useMutation({
    mutationFn: async (request: EvaluationRequest): Promise<ApiResponse<ModelEvaluation>> => {
      return qnnApiClient.runEvaluation(request);
    },
...
export function useCompareEvaluations(modelIds: string[]) {
  return useQuery({
    ...
    queryFn: async (): Promise<ModelEvaluation[]> => {
      // Fetch evaluations for all models in parallel
      const evaluations = await Promise.all(
        modelIds.map((id) => qnnApiClient.getEvaluation(id))
      );
      return evaluations;
    },
    staleTime: 5 * 60 * 1000,
